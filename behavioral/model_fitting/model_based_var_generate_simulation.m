...
output = cell(n_subj*n_sim, 1);
fit_params = cbm.output.parameters;
%fit_params = cbm.output.parameters{2};
...
for i = 1:n_subj
    for j = 1:n_sim
        ...
        subj2 = gen_var_RL_performance_sim(parameters, subj); % Target model
        output{(i-1)*n_sim+j} = subj2;
    end
end

output2 = jsonencode(output);
fileID = fopen('./model_based_var/model_based_generated_var_RL_performance_sim_indiv_aff_score_precise_rt_action_manual.json','w');
