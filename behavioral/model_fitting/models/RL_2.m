...
states = subj.state_list; % stim idxs in the order of cue1, cue2, cue1cue2, painting
...
for t=1:T
    ...
    p_pinch = exp(beta*q_pinch)/(exp(beta*q_pinch)+exp(beta*q_clench + beta2)+exp(beta*q_poke + beta3));
    p_clench = exp(beta*q_clench)/(exp(beta*q_pinch - beta2)+exp(beta*q_clench)+exp(beta*q_poke + beta3 - beta2));
