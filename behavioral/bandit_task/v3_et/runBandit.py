...
def runBandit(expInfo, dispInfo, tracker, taskInfo, taskObj, keyInfo, io):
    ...
    for sI in np.arange(taskInfo.numSessions):
        ...
        print('Attempting the initial connection to AWS')
...
def runTrial(sI, tI, taskObj, taskInfo, dispInfo, keyInfo, sessionInfo, sessionClock, condOfTrial, stream, outDir, tracker, io):
    ...
    def captureVideo(stop):
        stream2 = cv2.VideoCapture(CAM_CODE)
        stream2.set(cv2.CAP_PROP_AUTOFOCUS, 0)
        stream2.set(cv2.CAP_PROP_FPS, 60)
        ret, img = stream2.read()
        fps = stream2.get(cv2.CAP_PROP_FPS)
        fourcc = cv2.VideoWriter_fourcc('M','J','P','G')
        videoSave = cv2.VideoWriter(outDir + os.sep + 'videos' + os.sep + 'sess' + str(sI+1) +'_trial' + str(tI+1) +'.avi', fourcc, fps, (192, 108))
        while True:
            ret, img = stream2.read()
            if ret :
                img = rescale_frame(img, percent=10)
                ...
                cv2.putText(frame,
                        'time:' + str(round(sessionClock.getTime(), 3)),
                        (10, 10),
    ...
    stop_threads = False
    thread = threading.Thread(target=captureVideo, args=(lambda : stop_threads, ))
    thread.start()

    while (sessionClock.getTime() - stimOnset) <= taskInfo.trialInfo.maxRT:
        # process key response
        keyResponse = event.getKeys(keyList=['escape'])
