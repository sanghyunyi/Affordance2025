...
def initTask(expInfo):
    ...
    def dispParam(expInfo):
        ...
        print("eyetracker setup complete")
        xRes = 1280 #1440 #1366 1920
        yRes = 960 #900 #768 1080
    ...
    # TODO
    # To make all the participant sees the same stimuli, subID and dayNo are fixed but worth to reconsider
    subID = 2#expInfo.SubNo
    dayNo = 1#expInfo.dayNo
    ...
    # Responses (ISI)
    respFix = visual.TextStim(screen,
                              text="+",
                              font=dispInfo.textFont,
                              pos=screen.pos,
                              height=0.15,
    ...
    readyExp = visual.TextStim(screen,
                               text="Please get ready for the task. Waiting for experimenter...", #"Press " + keyInfo.instructDone + " to start",
                               font=dispInfo.textFont,
                               alignHoriz='center',
                               height=0.15,
                               color='white',
...
def initSessions(taskInfo, numSessions):
    ...
    for sI in range(taskInfo.numSessions):
        ...
        if average_session_lengths == numCond:
            ...
            subID = 6#taskInfo.subID
            dayNo = 1#taskInfo.dayNo
            key = subID * 937 + dayNo * 509
        ...
        print('session Id : ' + str(sI))
        print('session length: ', numSessTrials)
        print('Duration(s) : ' + str(numSessTrials*taskInfo.trialInfo.maxRT + sum(itiDur) + sum(isiDur) + sum(fbDur) + taskInfo.trialInfo.disDaqTime + 5))
        ...
        isOverrode = np.zeros(numSessTrials,dtype=bool)
...
def setup_eyetracker(expInfo):
    ...
    eyetracker_config['simulation_mode'] = False # TODO HAVE TO CHANGE THIS to False!
