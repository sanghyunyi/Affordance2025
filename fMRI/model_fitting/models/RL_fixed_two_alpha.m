...
alpha1     = 1/(1+exp(-nd_alpha1)); % alpha (transformed to be between zero and one)
...
nd_alpha2  = parameters(8); % normally-distributed alpha
alpha2     = 1/(1+exp(-nd_alpha2)); % alpha (transformed to be between zero and one)
...
shown_cond = subj.shownCond; % incongruent/congruent high/low
...
for t=1:T
    ...
    p_poke = exp(beta*q_poke+beta5)/(exp(beta*q_pinch+beta3)+exp(beta*q_clench+beta4)+exp(beta*q_poke+beta5));
    ...
    p_poke_aff =  exp(beta*aff_poke+beta5)/(exp(beta*aff_pinch+beta3)+exp(beta*aff_clench+beta4)+exp(beta*aff_poke+beta5));
